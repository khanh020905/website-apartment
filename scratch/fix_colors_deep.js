const fs = require('fs');
const path = require('path');

const replacements = [
    [/bg-amber-400\s+text-slate-900/g, 'bg-brand-primary text-white'],
    [/bg-amber-400\s+hover:bg-amber-500\s+text-slate-900/g, 'bg-brand-primary hover:bg-brand-dark text-white'],
    [/text-slate-900\s+bg-amber-400\s+hover:bg-amber-500/g, 'text-white bg-brand-primary hover:bg-brand-dark'],
    [/bg-linear-to-r\s+from-amber-400\s+to-yellow-400/g, 'bg-linear-to-r from-brand-primary to-brand-dark'],
    [/from-amber-400\s+to-yellow-400/g, 'from-brand-primary to-brand-dark'],
    [/from-amber-500\s+to-orange-500/g, 'from-brand-dark to-brand-primary'],
    [/bg-amber-400/g, 'bg-brand-primary'],
    [/hover:bg-amber-500/g, 'hover:bg-brand-dark'],
    [/text-amber-500/g, 'text-brand-primary'],
    [/text-amber-600/g, 'text-brand-dark'],
    [/text-amber-700/g, 'text-brand-ink'],
    [/text-yellow-500/g, 'text-brand-primary'],
    [/text-yellow-700/g, 'text-brand-ink'],
    [/bg-amber-50/g, 'bg-brand-bg'],
    [/bg-amber-100/g, 'bg-brand-bg'],
    [/bg-yellow-100/g, 'bg-brand-bg'],
    [/border-amber-100/g, 'border-brand-primary/20'],
    [/border-amber-200/g, 'border-brand-primary/20'],
    [/border-amber-300/g, 'border-brand-primary/30'],
    [/border-amber-400/g, 'border-brand-primary'],
    [/border-amber-500/g, 'border-brand-primary'],
    [/border-yellow-300/g, 'border-brand-primary/30'],
    [/border-t-amber-500/g, 'border-t-brand-primary'],
    [/shadow-amber-200\/50/g, 'shadow-brand-primary/20'],
    [/shadow-amber-400\/20/g, 'shadow-brand-primary/20'],
    [/shadow-amber-500\/25/g, 'shadow-brand-primary/20'],
    [/focus:border-amber-400/g, 'focus:border-brand-primary'],
    [/focus:border-amber-500/g, 'focus:border-brand-primary'],
    [/focus:ring-amber-500/g, 'focus:ring-brand-primary/20'],
    [/bg-\[#fcd34d\]/g, 'bg-brand-primary'],
    [/bg-brand-primary\s+text-slate-900/g, 'bg-brand-primary text-white'],
    [/text-slate-900\s+bg-brand-primary/g, 'text-white bg-brand-primary']
];

function processFile(filePath) {
    // Avoid Google icons and hardcoded logos if they contain yellow intentionally
    // In this project they seem to be in SVG tags.
    
    let content = fs.readFileSync(filePath, 'utf8');
    let original = content;
    
    for (const [regex, replacement] of replacements) {
        content = content.replace(regex, replacement);
    }
    
    if (content !== original) {
        fs.writeFileSync(filePath, content, 'utf8');
        console.log(`Updated: ${filePath}`);
        return true;
    }
    return false;
}

function walkDir(dir) {
    const files = fs.readdirSync(dir);
    for (const file of files) {
        const fullPath = path.join(dir, file);
        if (fs.statSync(fullPath).isDirectory()) {
            walkDir(fullPath);
        } else if (file.endsWith('.tsx') || file.endsWith('.ts') || file.endsWith('.css')) {
            processFile(fullPath);
        }
    }
}

const baseDir = path.join(__dirname, '..', 'client', 'src');
walkDir(baseDir);
console.log('Deep mass replacement complete.');
