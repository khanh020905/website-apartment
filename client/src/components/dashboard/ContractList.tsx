    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {contracts.map((contract, i) => (
        <motion.div
          ...
          className="bg-white rounded-[32px] border border-slate-100 p-6 shadow-sm hover:shadow-xl hover:shadow-indigo-900/5 transition-all group"
        >
          ...
          <div className="space-y-4 mb-8">
            ...
            <div className="grid grid-cols-2 gap-4 pt-2 border-t border-slate-50">
              ...
              <div>
                ...
                <p className="text-lg font-black text-indigo-600">{(contract.deposit_amount / 1000000).toFixed(1)}M</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="flex-1 py-3 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-800 transition-colors"
            >
              Chi tiết
            </button>
            <button 
              onClick={() => onDelete(contract.id)}
              className="w-12 h-12 border border-slate-100 text-slate-400 rounded-2xl flex items-center justify-center hover:bg-rose-50 hover:text-rose-600 hover:border-rose-100 transition-all cursor-pointer"
