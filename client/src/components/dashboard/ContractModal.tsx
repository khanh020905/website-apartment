    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
        ...
        <motion.div
          ...
          className="relative w-full max-w-2xl bg-white rounded-[40px] shadow-2xl shadow-slate-900/20 overflow-hidden"
        >
          {/* Header */}
          <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-gradient-to-r from-indigo-50/50 to-white">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <ShieldCheck className="w-5 h-5 text-indigo-600" />
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Rental Agreement</span>
              </div>
          ...
          <form onSubmit={handleSubmit} className="p-4 md:p-8 overflow-y-auto max-h-[70vh] grid grid-cols-1 md:grid-cols-2 gap-6 scrollbar-hide">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Họ tên khách *</label>
              <div className="relative group">
                <input 
                  type="text" required value={formData.tenant_name}
                  onChange={e => setFormData({ ...formData, tenant_name: e.target.value })}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-700 focus:outline-none focus:border-indigo-600 focus:bg-white transition-all pl-11"
            ...
            <div className="space-y-1">
              ...
              <div className="relative group">
                <input 
                  type="tel" value={formData.tenant_phone}
            ...
            <div className="space-y-1 col-span-full">
              ...
              <div className="relative group">
                <input 
                  type="email" value={formData.tenant_email}
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Tiền thuê (triệu/tháng) *</label>
              <div className="relative group">
                <input 
                  type="number" step="0.1" required value={formData.rent_amount}
                  onChange={e => setFormData({ ...formData, rent_amount: e.target.value })}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-700 focus:outline-none focus:border-indigo-600 focus:bg-white transition-all pl-11"
                  placeholder="3.5" 
                />
                <CreditCard className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300 group-focus-within:text-indigo-600 transition-colors" />
              </div>
            </div>

            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Tiền cọc (triệu)</label>
              <div className="relative group">
                <input 
                  type="number" step="0.1" value={formData.deposit_amount}
                  onChange={e => setFormData({ ...formData, deposit_amount: e.target.value })}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-700 focus:outline-none focus:border-indigo-600 focus:bg-white transition-all pl-11"
                  placeholder="3.5" 
                />
                <ShieldCheck className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300 group-focus-within:text-indigo-600 transition-colors" />
              </div>
            </div>
            ...
            <div className="col-span-full space-y-1 mt-4">
              ...
              <div className="relative group">
                <textarea 
                  ...
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-700 focus:outline-none focus:border-indigo-600 focus:bg-white transition-all resize-none pl-11"
                  placeholder="Ghi chú về cọc, điện nước, xe cộ..." 
                />
                <Notebook className="absolute left-4 top-6 w-4 h-4 text-slate-300 group-focus-within:text-indigo-600 transition-colors" />
              </div>
            </div>
            ...
            <div className="col-span-full pt-8 flex gap-4">
              <button 
                ...
                className="flex-1 py-4 border-2 border-slate-100 text-slate-400 rounded-3xl text-sm font-black uppercase tracking-widest hover:bg-slate-50 transition-all"
              >
                Hủy bỏ
              </button>
              <button 
                type="submit"
                className="flex-1 py-4 bg-indigo-600 text-white rounded-3xl text-sm font-black uppercase tracking-widest hover:bg-indigo-700 shadow-xl shadow-indigo-900/10 hover:scale-[1.02] transition-all cursor-pointer"
              >
