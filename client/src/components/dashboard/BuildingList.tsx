...
import { Building2, MapPin, Trash2, Edit3, QrCode } from "lucide-react";
				...
				{buildings.map((b) => (
					...
					>
						...
						<div className="flex items-start justify-between gap-4 relative z-10">
							<div
								className={`p-4 rounded-3xl transition-colors ${selectedId === b.id ? "bg-linear-to-br from-brand-primary to-brand-primary/80 text-white shadow-lg shadow-brand-primary/20" : "bg-slate-50 text-slate-400 group-hover:bg-brand-primary/5 group-hover:text-brand-primary"}`}
							>
							...
							<div className="flex-1 min-w-0">
								<h4
									className={`text-xl font-black truncate mb-1 transition-colors ${selectedId === b.id ? "text-brand-ink" : "text-slate-800"}`}
								>
									{b.name}
								</h4>
						...
						<div className="mt-6 flex items-center justify-between border-t border-slate-50 pt-4">
							...
							<div
								className={`text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-xl transition-all ${
									...
									:	"bg-slate-50 text-slate-400 group-hover:bg-brand-primary/10 group-hover:text-brand-primary"
								}`}
							>
