  ...
  return (
    <div className="fixed inset-0 z-100 flex items-center justify-center p-4">
      ...
      <motion.div 
        ...
        className="relative bg-white w-full max-w-2xl rounded-4xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
      >
        {/* Header */}
        ...
        <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-8 space-y-8 scrollbar-hide">
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Tên tòa nhà *</label>
            ...
            {/* Address */}
            <div className="space-y-3">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Địa chỉ đường *</label>
              <div className="relative">
            ...
            <div className="grid grid-cols-3 gap-4">
              {/* City */}
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Tỉnh / Thành phố</label>
                <input 
                  type="text" 
                  placeholder="TP. Hồ Chí Minh" 
                  value={formData.city}
                  onChange={e => setFormData({ ...formData, city: e.target.value })}
                  className="w-full h-14 px-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold placeholder-slate-300 focus:bg-white focus:border-brand-primary transition-all outline-none"
                />
              </div>
              {/* District */}
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Quận / Huyện</label>
                <input 
                  type="text" 
                  placeholder="Quận 7" 
              ...
              {/* Ward */}
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Phường / Xã</label>
                <input 
                  type="text" 
            ...
            <div className="grid grid-cols-2 gap-6">
              {/* Floors */}
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Số tầng hiển thị *</label>
                <div className="relative">
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Tọa độ Bản đồ (Lat/Lng)</label>
                <div className="flex gap-2">
                  <input 
                    ...
                    className="flex-1 h-14 px-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold placeholder-slate-300 focus:bg-white focus:border-brand-primary transition-all outline-none"
                  />
                  <input 
                    type="number" 
                    step="0.000001"
            ...
            {/* Description */}
            <div className="space-y-3">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Mô tả tòa nhà</label>
              <textarea 
                rows={4}
                placeholder="VD: Tòa nhà gần chợ, an ninh tốt, có bảo vệ 24/7..."
                value={formData.description}
                onChange={e => setFormData({ ...formData, description: e.target.value })}
                className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold placeholder-slate-300 focus:bg-white focus:border-brand-primary transition-all outline-none resize-none text-sm"
              />
            </div>
          </div>
        </form>
        ...
        <div className="p-8 border-t border-slate-50 flex gap-4 bg-slate-50/20">
          ...
          <button onClick={handleSave} className="flex-1 h-14 rounded-2xl bg-brand-primary text-white font-black text-xs uppercase tracking-widest shadow-xl shadow-brand-primary/20 hover:bg-brand-dark transition-colors cursor-pointer">
