...
const SummaryCard = ({ label, value, icon, color, trend }: SummaryCardProps) => (
  <motion.div 
    ...
    className="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm hover:shadow-md transition-shadow"
  >
...
export const Overview = ({ stats }: OverviewProps) => {
  ...
  const formatPrice = (p: number) => {
    return new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(p).replace('₫', '').trim() + ' ₫';
  };

  const revTrend = stats.revenue.last > 0 
    ? { value: Math.round(((stats.revenue.current - stats.revenue.last) / stats.revenue.last) * 100), isUp: stats.revenue.current >= stats.revenue.last }
    : undefined;
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Xu hướng 6 tháng gần nhất</p>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 bg-emerald-50 rounded-xl">
              <TrendingUp className="w-4 h-4 text-emerald-600" />
              <span className="text-sm font-black text-emerald-700">{Math.round(stats.occupancyRate)}%</span>
            </div>
          </div>
            ...
            <div className="space-y-4">
              {stats.expiringContracts.d7.length > 0 && (
                <div className="p-4 bg-rose-50 rounded-2xl border border-rose-100">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[10px] font-black text-rose-600 uppercase tracking-widest">Trong 7 ngày tới</span>
                    <AlertCircle className="w-4 h-4 text-rose-500" />
                  </div>
                  <p className="text-sm font-black text-rose-900">{stats.expiringContracts.d7.length} hợp đồng cần gia hạn</p>
                </div>
              )}
              ...
              {stats.expiringContracts.d30.length > 0 ? (
                  ...
                  {stats.expiringContracts.d30.slice(0, 4).map((c: Contract) => (
                    <div key={c.id} className="flex items-center justify-between p-3 rounded-xl border border-slate-50 hover:bg-slate-50 transition-colors">
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">{c.end_date}</p>
                      </div>
                ...
                <div className="py-10 text-center space-y-3">
                  ...
                  <p className="text-sm font-bold text-slate-400">Không có hợp đồng nào sắp hết hạn</p>
                </div>
              )}
            </div>
          </div>
