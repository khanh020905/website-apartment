...
export default function ContractTemplateForm({ onSubmit, onCancel }: ContractTemplateFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col h-[calc(100vh-140px)] max-h-[700px]">
      <div className="flex-1 overflow-y-auto p-6 space-y-8 scrollbar-thin scrollbar-thumb-slate-200">
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
            ...
            <h3 className="text-sm font-bold text-slate-900 uppercase tracking-tight">Thông tin cơ bản</h3>
          </div>
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
            ...
            <h3 className="text-sm font-bold text-slate-900 uppercase tracking-tight">File mẫu hợp đồng</h3>
          </div>
          ...
          <div className="border-2 border-dashed border-slate-200 rounded-2xl p-8 flex flex-col items-center justify-center text-center hover:bg-slate-50/50 hover:border-amber-400 transition-colors cursor-pointer group">
             ...
             <p className="text-sm font-bold text-slate-700 mb-1">Kéo thả tệp vào đây hoặc click để tải lên</p>
             <p className="text-xs font-medium text-slate-500">Chỉ hỗ trợ tệp định dạng .docx (Word)</p>
             <button type="button" className="mt-4 px-4 py-2 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-700 shadow-sm hover:border-amber-400 hover:text-amber-600 transition-all">
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between pb-2 border-b border-slate-100">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-slate-900 uppercase tracking-tight">Danh sách thẻ dữ liệu (Tags)</h3>
            </div>
          </div>

          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
            <p className="text-xs text-slate-600 font-medium mb-3">
              Vui lòng chèn các nhãn chính xác (bao gồm cả dấu ngoặc nhọn) vào nội dung file Word của bạn. Hệ thống sẽ tự động thay thế bằng dữ liệu thực tế khi xuất hợp đồng:
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {TAGS.map((tag, idx) => (
                <div key={idx} className="flex items-center gap-2 p-2 bg-white border border-slate-100 rounded-lg text-xs font-medium text-slate-700 hover:border-amber-200 transition-colors cursor-default">
                  <span className="w-1.5 h-1.5 rounded-full bg-amber-400 shrink-0"></span>
                  <code className="font-mono text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded text-[11px] font-bold">{tag.split(" - ")[0]}</code>
