...
const ContractForm = ({ onSubmit, onCancel }: ContractFormProps) => {
  const [formData, setFormData] = useState({
    ...
    contractName: '',
    contractNumber: 'NTSMT' + Math.floor(Math.random() * 100000).toString().padStart(5, '0'),
    effectiveDate: '',
    startDate: '',
    endDate: '',
    room: '',
  ...
  return (
    <form id="contract-form" onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100">
        ...
        />
        <label htmlFor="isSigned" className="text-sm font-bold text-slate-700">Đã ký hợp đồng</label>
      </div>
      ...
      <div className="space-y-2">
        <label className="text-[11px] font-black uppercase tracking-wider text-slate-400">Ghi chú</label>
        <textarea
          name="notes"
