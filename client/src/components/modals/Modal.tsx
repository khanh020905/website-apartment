...
const Modal = ({ isOpen, onClose, title, children, footer, size = 'md' }: ModalProps) => {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 overflow-y-auto bg-slate-900/40 backdrop-blur-[2px] transition-all" onClick={handleBackdropClick}>
          <motion.div
          ...
          >
            ...
            {/* Footer */}
            {footer && (
              <div className="px-8 py-4 border-t border-slate-50 bg-slate-50/30 flex items-center justify-end gap-3 flex-shrink-0">
                {footer}
              </div>
            )}
          </motion.div>
