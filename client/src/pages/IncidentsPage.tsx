...
export default function IncidentsPage() {
  ...
  const filteredData = MOCK_INCIDENTS.filter((t) => {
    if (search && !t.location.toLowerCase().includes(search.toLowerCase()) && !t.reportedBy.toLowerCase().includes(search.toLowerCase())) return false;
          ...
          <div className="overflow-x-auto flex-1">
            <table className="w-full h-full text-left">
              <thead className="bg-[#f8f9fa] border-b border-slate-200 sticky top-0 z-10">
                <tr>
                  {[
                    "Loại sự cố", "Ưu tiên", "Vị trí sự cố", "Trạng thái", "Báo cáo bởi", "Người đảm nhận"
                  ].map((h, i) => (
                ...
                {filteredData.length === 0 ? (
                  ...
                  filteredData.map((row: Incident) => (
                    <tr key={row.id} className="hover:bg-amber-50/20 transition-colors group">
                      ...
                      <td className="px-5 py-3.5">
                         <span className="text-[13px] font-medium text-slate-600">{row.priority}</span>
                      </td>
                      <td className="px-5 py-3.5">
                         <span className="text-[13px] font-medium text-slate-600">{row.location}</span>
                      </td>
                      <td className="px-5 py-3.5">
                      ...
                      <td className="px-5 py-3.5">
                         <span className="text-[13px] font-medium text-slate-600">{row.reportedBy}</span>
                      </td>
                      <td className="px-5 py-3.5">
                         <span className="text-[13px] font-medium text-slate-600">{row.assignee}</span>
                      </td>
