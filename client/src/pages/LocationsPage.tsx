...
import { Plus, Search, MapPin, Phone, Building2, MoreHorizontal, Settings, Edit2, Trash2, X, Home, Map as MapIcon, Building, Bed, Briefcase, Zap, Droplet, Bike, ShieldCheck, Camera, Wind, Waves, User, Clock, Monitor, Heart, Shield, Trash, ChevronLeft, Navigation, Wifi, Car, Flame, ArrowUpCircle, Box, ChevronDown } from "lucide-react";
                    ...
                    <tr key={loc.id} className="hover:bg-brand-bg/20 transition-colors group relative">
                       ...
                       <td className="px-5 py-4">
                          <div className="flex flex-wrap gap-1.5 max-w-[200px]">
                             {/* Optimized with System Brand Colors */}
                             {((loc as any).fixed_services?.length > 0 || (loc as any).metered_services?.length > 0) ? (
                                <>
                                  {(loc as any).metered_services?.map((s: any, i: number) => (
                                     <span key={`m-${i}`} className="px-2 py-0.5 rounded text-[11px] font-bold bg-brand-bg text-brand-primary border border-brand-light/30">
                                        {s.name}
                                     </span>
                                  ))}
                                  {(loc as any).fixed_services?.map((s: any, i: number) => (
                                     <span key={`f-${i}`} className="px-2 py-0.5 rounded text-[11px] font-bold bg-emerald-50 text-emerald-600 border border-emerald-100">
                                        {s.name}
                                     </span>
                                  ))}
                                </>
