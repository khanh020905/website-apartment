			...
			<div className="flex-1 overflow-auto bg-[#f8f9fa] p-5 lg:p-6 flex flex-col gap-6">
				...
				<div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm h-full flex flex-col min-h-100">
					<div className="overflow-x-auto flex-1">
						<table className="w-full text-left h-full">
							<thead className="bg-[#f8f9fa] border-b border-slate-200 sticky top-0">
								<tr>
									{["Tên toà nhà", "Loại hình", "Tổng phòng", "Phòng trống", "Địa chỉ"].map(
										(h, i) => (
											<th
												key={i}
							...
							<tbody className="divide-y divide-slate-100">
								{MOCK_DATA.map((row) => (
									...
									>
										...
										<td className="px-5 py-4 whitespace-nowrap">
											<span className="text-[13px] font-medium text-slate-600 px-2 py-1 bg-slate-50 rounded border border-slate-200">
												{row.type}
											</span>
										</td>
										<td className="px-5 py-4 text-[13px] font-bold text-slate-900 whitespace-nowrap">
											{row.totalUnits}
										</td>
										<td className="px-5 py-4 whitespace-nowrap">
											<span
												className={`px-2 py-0.5 rounded-md text-[11px] font-bold uppercase tracking-wider border ${row.vacantUnits > 5 ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-amber-50 text-amber-700 border-amber-200"}`}
											>
					...
					<div className="mt-auto px-5 py-3 border-t border-slate-200 bg-white flex items-center justify-end gap-4 text-[13px] text-slate-600">
						<span>
							1-{MOCK_DATA.length}/{MOCK_DATA.length} kết quả
						</span>
