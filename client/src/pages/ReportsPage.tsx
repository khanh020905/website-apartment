		...
		<div className="p-6 lg:p-8 space-y-6 bg-linear-to-br from-slate-50 to-slate-100/50 min-h-full">
			...
			<div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
				...
				<button
					onClick={fetchData}
					disabled={loading}
					className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-600 hover:bg-slate-50 shadow-sm transition-all disabled:opacity-50"
				>
					...
					<div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
						{statCards.map((stat, i) => (
							<motion.div
								...
								className={`relative overflow-hidden bg-linear-to-br ${stat.gradient} p-5 rounded-2xl shadow-lg ${stat.shadow} hover:scale-[1.02] transition-transform`}
							>
								<div className="flex items-center justify-between">
									<div>
										<p className="text-2xl lg:text-3xl font-black text-white mb-0.5">
											{stat.value}
										</p>
											...
											>
												...
												<td className="px-4 py-3">
													<div className="flex items-center gap-3">
														...
														<span className="text-sm font-bold text-brand-primary w-10 text-right">
															{b.rate}%
														</span>
													</div>
												</td>
										...
										{displayData.room_details.map((r, idx) => (
											...
											>
												...
												<td className="px-4 py-3">
													<div className="flex items-center gap-1">
														<Calendar className="w-3.5 h-3.5 text-slate-400" />
														<span className="text-sm text-slate-600">{formatDate(r.end_date)}</span>
													</div>
												</td>
												<td className="px-4 py-3 text-sm font-semibold text-slate-800">
													{r.rent > 0 ? formatCurrency(r.rent) : "—"}
												</td>
