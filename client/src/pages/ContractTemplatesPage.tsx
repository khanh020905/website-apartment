...
const MOCK_TEMPLATES: ContractTemplate[] = [
  { id: "1", name: "Mẫu hợp đồng thuê căn hộ chuẩn", short_name: "HD-STUDIO", prefix_code: "HD{YYYY}{MM}", reminder_days: 30, file_name: "HD-Chuan.docx", status: "active" },
  { id: "2", name: "Mẫu hợp đồng thuê văn phòng", short_name: "HD-VP", prefix_code: "HDVP{MM}", reminder_days: 15, file_name: "HD-Vanphong.docx", status: "active" },
  { id: "3", name: "Mẫu biên bản bàn giao", short_name: "BBBG", prefix_code: "BBBG", reminder_days: 0, file_name: "BB-Bangiao.docx", status: "inactive" },
];
      ...
      <div className="flex-1 overflow-auto bg-[#f8f9fa] p-6 lg:p-8 flex flex-col gap-6">
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm h-full flex flex-col min-h-[400px]">
            ...
            <table className="w-full h-full text-left">
              <thead className="bg-[#f8f9fa] border-b border-slate-200 sticky top-0 z-10">
                <tr>
                  ...
                  {[
                    "Tên mẫu hợp đồng", "Tên viết tắt", "Mẫu số hợp đồng", "Nhắc nhở kỳ hạn", "File mẫu hợp đồng", "Trạng thái",
                  ].map((h, i) => (
                ...
                {filteredData.length === 0 ? (
                  ...
                  filteredData.map((row) => (
                    <tr key={row.id} className="hover:bg-amber-50/20 transition-colors group">
                      ...
                      <td className="px-5 py-3.5">
                        <span className="text-[13px] font-bold text-blue-600 truncate max-w-[200px] block" title={row.name}>
                           {row.name}
                        </span>
                      </td>
                      <td className="px-5 py-3.5">
                        <span className="font-mono text-[12px] font-bold text-slate-700">{row.short_name || "—"}</span>
                      </td>
                      <td className="px-5 py-3.5">
                         <span className="font-mono text-[12px] font-bold text-slate-700">{row.prefix_code || "—"}</span>
                      </td>
                      ...
                      <td className="px-5 py-3.5">
                        <span className={`inline-flex px-2 py-0.5 rounded-md text-[11px] font-bold uppercase tracking-wide border ${
                           row.status === "active" 
          ...
          <div className="mt-auto px-5 py-3 border-t border-slate-200 bg-white flex items-center justify-end gap-4 text-[13px] text-slate-600">
             <span>{filteredData.length === 0 ? "0-0/0" : `1-${filteredData.length}/${filteredData.length}`}</span>
