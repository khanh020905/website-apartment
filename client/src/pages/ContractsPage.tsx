...
export default function ContractsPage() {
  ...
  const STATS = [
    { label: "Đang hiệu lực", value: 124, icon: CheckCircle2, color: "text-emerald-500", bg: "bg-emerald-50" },
      ...
      <div className="bg-white border-b border-slate-200">
        ...
        <div className="px-6 pb-4 flex flex-col sm:flex-row items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <input type="date" className="w-[125px] px-3 py-2 border border-slate-200 rounded-lg text-sm text-slate-600 bg-white focus:outline-none focus:border-amber-400 hover:border-slate-300 transition-all font-medium text-[13px] " />
      ...
      <div className="flex-1 overflow-auto bg-[#f8f9fa] p-5 lg:p-6 flex flex-col gap-6">
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm h-full flex flex-col min-h-[400px]">
          <div className="overflow-x-auto flex-1">
            <table className="w-full h-full text-left">
              <thead className="bg-[#f8f9fa] border-b border-slate-200 sticky top-0 z-10">
                <tr>
                  {["Mã hợp đồng", "Tên hợp đồng", "Phòng", "Khách ở", "Thời hạn", "Trạng thái"].map((h, i) => (
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-28 text-center bg-white cursor-default">
                        <div className="w-8 h-8 border-4 border-amber-200 border-t-amber-500 rounded-full animate-spin mx-auto mb-3" />
                        <p className="text-sm font-medium text-slate-500 uppercase tracking-widest">Đang tải...</p>
                    </td>
                  </tr>
                ) : (
                  contracts.map((c) => (
                    <tr key={c.id} className="hover:bg-amber-50/20 transition-colors group">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[13px] font-bold text-slate-900 truncate max-w-[200px]">{c.contract_name}</span>
                        </div>
                      </td>
                      <td className="px-5 py-4 whitespace-nowrap">
                      ...
                      <td className="px-5 py-4 whitespace-nowrap">
                        <span className={`px-2 py-0.5 rounded-md text-[11px] font-bold uppercase tracking-wider border ${STATUS_MAP[c.status].color.replace('bg-', 'bg-opacity-20 border-').replace('text-', 'text-')}`}>
          ...
          <div className="mt-auto px-5 py-3 border-t border-slate-200 bg-white flex items-center justify-end gap-4 text-[13px] text-slate-600">
             <span>{contracts.length === 0 ? "0-0/0" : `1-${contracts.length}/${contracts.length}`} kết quả</span>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-6">
                 ...
                 <div>
                   ...
                   <div className="grid grid-cols-2 gap-3">
                     {["Hết hiệu lực", "Hiệu lực", "Chờ tái ký", "Không tái ký", "Đã thanh lý", "Bản thảo"].map(st => (
                        <label key={st} className="flex items-center gap-2 cursor-pointer">
                           <input type="checkbox" className="rounded border-slate-300 text-amber-500 focus:ring-amber-500 w-4 h-4 cursor-pointer" />
                           <span className="text-[13px] font-medium text-slate-700">{st}</span>
                        </label>
