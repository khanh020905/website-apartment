   ...
   return (
      <div className="flex flex-col gap-6">
         ...
         <div className="grid grid-cols-1 md:grid-cols-4 gap-4 shrink-0">
           {STATS.map((stat, i) => (
             <div key={i} className="bg-white px-5 py-4 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-center gap-1.5 min-h-[90px]">
             ...
             <div className="flex-1 p-5">
                <ResponsiveContainer width="100%" height="100%">
                   <BarChart data={MOCK_MONTHLY} margin={{top: 10, right: 10, left: -20, bottom: 0}}>
                      ...
                      <Bar dataKey="room" name="Tiền phòng" stackId="a" fill="#3b82f6" radius={[0,0,4,4]} barSize={30} />
            ...
            <div className="overflow-x-auto flex-1 border border-slate-200 rounded-lg">
               <table className="w-full text-left">
                 <thead className="bg-[#f8f9fa] border-b border-slate-200">
                   <tr>
                     <th className="px-4 py-3 text-[12px] font-bold text-slate-600 tracking-wide whitespace-nowrap border-r border-slate-200 sticky left-0 z-10 bg-[#f8f9fa]">Nhóm dịch vụ</th>
                     {Array.from({length: 12}).map((_, i) => (
                        <th key={i} className="px-4 py-3 text-[12px] font-bold text-slate-600 tracking-wide whitespace-nowrap text-right">T{i+1}</th>
                     ))}
                     <th className="px-4 py-3 text-[12px] font-bold text-slate-600 tracking-wide whitespace-nowrap text-right bg-amber-50">Tổng cộng</th>
                     <th className="px-4 py-3 text-[12px] font-bold text-slate-600 tracking-wide whitespace-nowrap text-right">% Tổng</th>
                   </tr>
                 </thead>
                 <tbody className="divide-y divide-slate-100">
                    {['Tiền phòng', 'Tiện ích (Điện, Nước)', 'Dịch vụ bổ sung', 'Khác'].map((s, idx) => (
                       <tr key={idx} className="hover:bg-slate-50 transition-colors">
                          ...
                          {Array.from({length: 12}).map((_, i) => (
                             <td key={i} className="px-4 py-3 text-[12px] text-slate-600 whitespace-nowrap text-right font-medium">
...
const RentalRevenueTab = () => {
   return (
      <div className="flex flex-col gap-6">
         {/* Charts */}
         <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col h-[400px] shrink-0">
            <div className="p-5 border-b border-slate-100 flex items-center justify-between">
            ...
            <div className="flex-1 p-5">
               <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={MOCK_MONTHLY} margin={{top: 10, right: 10, left: -20, bottom: 0}}>
                     ...
                     <Line type="monotone" dataKey="total" name="Tổng doanh thu" stroke="#f59e0b" strokeWidth={3} dot={{r: 4, strokeWidth: 2}} activeDot={{r: 6}} />
                  </LineChart>
               </ResponsiveContainer>
            </div>
         </div>
         ...
         <div className="bg-white p-5 lg:p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col flex-1 min-h-[400px]">
            ...
            <div className="overflow-x-auto flex-1 border border-slate-200 rounded-lg">
               <table className="w-full text-left">
                 <thead className="bg-[#f8f9fa] border-b border-slate-200">
                   <tr>
                     <th className="px-4 py-3 text-[12px] font-bold text-slate-600 tracking-wide whitespace-nowrap border-r border-slate-200 sticky left-0 z-10 bg-[#f8f9fa]">Tòa nhà</th>
                     {Array.from({length: 12}).map((_, i) => (
                        <th key={i} className="px-4 py-3 text-[12px] font-bold text-slate-600 tracking-wide whitespace-nowrap text-right">T{i+1}</th>
                     ))}
                     <th className="px-4 py-3 text-[12px] font-bold text-slate-600 tracking-wide whitespace-nowrap text-right bg-amber-50">Tổng cộng</th>
   ...
   return (
      <div className="flex flex-col gap-6">
         ...
         <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col h-[400px] shrink-0">
            ...
            <div className="flex-1 p-5">
               <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={MOCK_MONTHLY.slice(0, 6)} margin={{top: 10, right: 10, left: -20, bottom: 0}}>
                     <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                     <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fontSize: 12, fill: '#64748b'}} dy={10} />
                     <YAxis axisLine={false} tickLine={false} tick={{fontSize: 12, fill: '#64748b'}} />
                     <Tooltip cursor={{fill: '#f1f5f9'}} contentStyle={{borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)'}} />
                     <Legend iconType="circle" wrapperStyle={{fontSize: '12px', paddingTop: '10px'}} />
                     <Bar dataKey="room" name="Đã thanh toán" fill="#10b981" radius={[4,4,0,0]} barSize={20} />
                     <Bar dataKey="service" name="Chờ thanh toán" fill="#f59e0b" radius={[4,4,0,0]} barSize={20} />
                     <Bar dataKey="utility" name="Quá hạn" fill="#ef4444" radius={[4,4,0,0]} barSize={20} />
                  </BarChart>
               </ResponsiveContainer>
            </div>
         </div>
         ...
         <div className="bg-white p-5 lg:p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col flex-1 min-h-[400px]">
            ...
            <div className="overflow-x-auto flex-1 border border-slate-200 rounded-lg">
               <table className="w-full text-left">
                 <thead className="bg-[#f8f9fa] border-b border-slate-200">
                   <tr>
                     {["Mã hóa đơn", "Khách hàng", "Toà nhà/Phòng", "Số tiền (VNĐ)", "Trạng thái", "Ngày tạo", ""].map((h, i) => (
                        <th key={i} className="px-4 py-3 text-[12px] font-bold text-slate-600 tracking-wide whitespace-nowrap">{h}</th>
                     ))}
                   </tr>
                 </thead>
                 <tbody className="divide-y divide-slate-100">
                    {[
                       { id: 'INV-2604-001', cust: 'Nguyễn Văn A', room: 'Central - 101', amt: '5,500,000', status: 'Đã thanh toán', date: '01/04/2026' },
                       { id: 'INV-2604-002', cust: 'Trần Thị B', room: 'Sunrise - 305', amt: '7,200,000', status: 'Chờ thanh toán', date: '02/04/2026' },
                       { id: 'INV-2604-003', cust: 'Lê Hoàng C', room: 'Biệt thự B - 02', amt: '15,000,000', status: 'Quá hạn', date: '25/03/2026' },
                    ].map((row, idx) => (
                       <tr key={idx} className="hover:bg-slate-50">
                          ...
                          <td className="px-4 py-3">
                             <span className={`inline-flex px-2 py-0.5 rounded-md text-[11px] font-bold uppercase tracking-wider border ${
                                row.status === 'Đã thanh toán' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
...
const CashFlowTab = () => {
   const STATS = [
      ...
      { label: "Tổng tiền vào", value: "850,000,000", color: "text-emerald-600", bg: "bg-emerald-50" },
   ...
   return (
      <div className="flex flex-col gap-6">
         ...
         <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col h-[400px] shrink-0">
            <div className="p-5 border-b border-slate-100 flex items-center justify-between">
               <h3 className="text-[14px] font-bold text-slate-800">Biểu đồ dòng tiền (In/Out) - Triệu VNĐ</h3>
            </div>
            <div className="flex-1 p-5">
               <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={DAILY_FLOW} margin={{top: 10, right: 10, left: -20, bottom: 0}}>
                     <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                     <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fontSize: 12, fill: '#64748b'}} dy={10} />
                     <YAxis axisLine={false} tickLine={false} tick={{fontSize: 12, fill: '#64748b'}} />
                     <Tooltip cursor={{stroke: '#cbd5e1', strokeWidth: 1, strokeDasharray: '4 4'}} contentStyle={{borderRadius: '8px', border: 'none'}} />
                     <Legend iconType="circle" wrapperStyle={{fontSize: '12px', paddingTop: '10px'}} />
                     <Line type="monotone" dataKey="in" name="Tiền vào (In)" stroke="#10b981" strokeWidth={3} dot={{r: 4, fill: '#10b981'}} activeDot={{r: 6}} />
                     <Line type="monotone" dataKey="out" name="Tiền ra (Out)" stroke="#ef4444" strokeWidth={3} dot={{r: 4, fill: '#ef4444'}} activeDot={{r: 6}} />
                  </LineChart>
               </ResponsiveContainer>
            </div>
         </div>
         ...
         <div className="bg-white p-5 lg:p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col flex-1 min-h-[400px]">
            ...
            <div className="overflow-x-auto flex-1 border border-slate-200 rounded-lg">
               <table className="w-full text-left">
                 <thead className="bg-[#f8f9fa] border-b border-slate-200">
                   <tr>
                     {["Tòa nhà", "Phòng", "Dòng tiền", "Nhóm giao dịch", "Loại", "Số tiền (VNĐ)", "Ngày", "Hình thức"].map((h, i) => (
                        <th key={i} className="px-4 py-3 text-[12px] font-bold text-slate-600 tracking-wide whitespace-nowrap">{h}</th>
                     ))}
                   </tr>
                 </thead>
                 <tbody className="divide-y divide-slate-100">
                    {[
                       { build: 'Central', room: '101', flow: 'IN', group: 'Thu tiền khách', type: 'Tiền phòng', amt: '5,500,000', date: '01/04', method: 'Chuyển khoản' },
                       { build: 'Sunrise', room: '305', flow: 'IN', group: 'Thu tiền khách', type: 'Dịch vụ', amt: '1,200,000', date: '02/04', method: 'Tiền mặt' },
                       { build: 'Central', room: 'Chung', flow: 'OUT', group: 'Chi phí', type: 'Bảo trì', amt: '-2,500,000', date: '03/04', method: 'Chuyển khoản' },
                    ].map((row, idx) => (
                       <tr key={idx} className="hover:bg-slate-50">
                          <td className="px-4 py-3 text-[13px] font-bold text-slate-900">{row.build}</td>
                          <td className="px-4 py-3 text-[13px] text-slate-600">{row.room}</td>
                          <td className="px-4 py-3">
                             <span className={`inline-flex px-2 py-0.5 rounded-md text-[11px] font-bold uppercase tracking-wider border opacity-90 ${row.flow === 'IN' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200'}`}>
                                {row.flow}
                             </span>
                          </td>
                          <td className="px-4 py-3 text-[13px] text-slate-700">{row.group}</td>
                          <td className="px-4 py-3 text-[13px] text-slate-500">{row.type}</td>
                          <td className={`px-4 py-3 text-[13px] font-bold ${row.flow === 'IN' ? 'text-emerald-600' : 'text-rose-600'}`}>{row.amt}</td>
                          <td className="px-4 py-3 text-[13px] text-slate-500">{row.date}</td>
