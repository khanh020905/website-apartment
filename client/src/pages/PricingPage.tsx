      ...
      <div className="max-w-7xl mx-auto px-6 py-12 md:py-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-20">
          {PRICING_PLANS.map((plan, idx) => {
            const isCurrentPlan = user?.profile?.role === plan.role && 
              ...
              <motion.div
                ...
                className={`relative bg-white rounded-[40px] p-8 transition-all duration-300 flex flex-col ${
                  isCurrentPlan 
                    ...
                    : 'border-2 border-slate-100 hover:border-slate-300 hover:shadow-xl hover:shadow-slate-900/[0.03]'
                }`}
              >
                {isCurrentPlan && (
                  ...
                  {FEATURE_LABELS.slice(0, 6).map((f) => {
                    ...
                    return (
                      <div key={f.key} className="flex items-start gap-3">
                        <div className={`mt-0.5 rounded-full p-0.5 ${val === 'Khong' ? 'text-slate-300' : 'text-teal-600 bg-teal-50'}`}>
                          {val === 'Khong' ? <X size={14} /> : val === 'Han che' ? <Minus size={14} /> : <Check size={14} strokeWidth={4} />}
                        </div>
                        <div className="flex flex-col">
                          <span className="text-xs text-slate-400 font-bold uppercase tracking-wider">{f.label}</span>
                          <span className={`text-sm font-black ${val === 'Khong' ? 'text-slate-400 line-through opacity-50' : 'text-slate-800'}`}>
                            {val === 'Co' ? 'Có' : val === 'Khong' ? 'Không' : 'Hạn chế'}
                          </span>
                        </div>
                      </div>
                    );
            ...
            <table className="w-full text-left min-w-[800px]">
              <thead>
                <tr className="bg-slate-900 text-white">
                  <th className="p-6 font-black text-sm uppercase tracking-widest border-r border-white/10">Tính năng</th>
                  {PRICING_PLANS.map(p => (
                    <th key={p.id} className="p-6 text-center font-black text-sm uppercase tracking-widest border-r border-white/10 last:border-0">
                      {p.name}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {FEATURE_LABELS.map((f, idx) => (
                  <tr key={f.key} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                    <td className="p-6 text-sm font-bold text-slate-600 border-r border-slate-100">{f.label}</td>
                    {PRICING_PLANS.map(p => {
                      const val = p.features[f.key as keyof typeof p.features];
                      const isCurrentPackage = user?.profile?.role === p.role && 
                                              (user?.profile?.subscription === p.tier || 
                                               (!user?.profile?.subscription && p.tier === 'free'));
                      return (
                        <td key={p.id} className={`p-6 text-center border-r border-slate-100 last:border-0 ${isCurrentPackage ? 'bg-slate-50/50' : ''}`}>
                          {val === 'Co' ? (
                            <div className="flex justify-center"><Check className="text-teal-600" size={20} strokeWidth={3} /></div>
                          ) : val === 'Khong' ? (
                            <div className="flex justify-center"><X className="text-slate-300" size={20} /></div>
                          ) : val === 'Han che' ? (
                            <div className="flex justify-center"><Minus className="text-amber-500" size={20} strokeWidth={3} /></div>
                          ) : (
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-tight">{val}</span>
                          )}
                        </td>
                      );
                    })}
                ...
                <tr className="bg-indigo-50/30">
                  <td className="p-6 text-sm font-bold text-slate-600 border-r border-slate-100">Cơ chế kiểm duyệt</td>
                  <td className="p-6 text-center border-r border-slate-100 italic text-[10px] text-slate-400 font-bold px-4">Tìm kiếm công khai</td>
                  <td colSpan={2} className="p-6 text-center italic text-[10px] text-indigo-600 font-bold px-10 leading-relaxed uppercase tracking-wider">
                    "Kiểm duyệt tài khoản chủ nhà hay môi giới, nếu thông qua kiểm duyệt thì không cần kiểm duyệt tin đăng"
                  </td>
                </tr>
              </tbody>
            </table>
