    ...
    <div className="flex-1 flex overflow-hidden bg-slate-50">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8">
          <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20">
            <p className="text-white/90 text-[15px] leading-relaxed mb-5 italic">
              "Tìm phòng trọ chưa bao giờ dễ dàng đến thế. 
              Chỉ cần mở HomeSpot, chọn khu vực và ngân sách — 
              mọi thứ hiện ra ngay trên bản đồ. Tiết kiệm thời gian cực kỳ!"
            </p>
      ...
      >
        <div className="w-full max-w-[420px]">
          ...
          {/* Google Login */}
          <button className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-all mb-6 cursor-pointer">
          ...
          <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2.5 cursor-pointer" onClick={() => setRemember(!remember)}>
                <div className={`w-[18px] h-[18px] rounded border-2 flex-shrink-0 flex items-center justify-center transition-all ${
                  remember ? 'bg-emerald-700 border-emerald-700' : 'border-slate-300 hover:border-slate-400'
                }`}>
                  {remember && (
              ...
              <a href="#" className="text-sm text-emerald-700 font-medium hover:text-emerald-800 transition-colors">
                Quên mật khẩu?
              </a>
            </div>
          ...
          <p className="text-center text-sm text-slate-500 mt-6">
            Chưa có tài khoản?{' '}
            <Link to="/register" className="text-emerald-700 font-semibold hover:text-emerald-800 underline underline-offset-2 transition-colors">
              Đăng ký ngay
            </Link>
          </p>
        </div>
