...
export default function ListingDetailPage() {
	...
	if (!listing)
		return (
			<div className="flex-1 flex items-center justify-center">
				<p className="text-slate-500">Không tìm thấy tin đăng</p>
			</div>
		);
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
					...
					<div className="lg:col-span-2 space-y-8">
						...
						<motion.div
							...
							className="bg-white rounded-4xl overflow-hidden border border-slate-100 shadow-xl shadow-slate-200/50"
						>
							<div className="relative h-64 md:h-105 group">
								<img
								...
								<div className="flex gap-3 p-4 overflow-x-auto scrollbar-hide bg-slate-50/50 border-t border-slate-100">
									{images.map((img: string, i: number) => (
										<button
											...
											className={`w-20 h-16 rounded-2xl overflow-hidden shrink-0 cursor-pointer border-4 transition-all ${
												i === activeImage ?
													"border-teal-600 scale-105 shadow-lg shadow-teal-900/10"
						...
						<motion.div
							...
							className="bg-white rounded-4xl p-6 md:p-8 border border-slate-100 shadow-sm"
						>
							<div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-6">
								<div className="space-y-2">
									<div className="flex items-center gap-2">
										<span className="px-3 py-1 bg-teal-50 text-teal-700 text-[10px] font-black uppercase tracking-widest rounded-full">
											Tin đăng mới
										</span>
								...
								<div className="flex flex-col items-end">
									<div className="text-3xl font-black text-teal-700">
										{(listing.price / 1000000).toFixed(1)} <span className="text-sm">Trđ/th</span>
									</div>
									<p className="text-[10px] text-slate-300 font-bold uppercase tracking-widest mt-1">
										Đã bao gồm phí quản lý
									</p>
								</div>
							</div>
							...
							{listing.lat && listing.lng && (
								<div className="mb-4">
									...
									<div className="h-72 rounded-4xl overflow-hidden border-4 border-white shadow-xl">
										<MapContainer
									...
									<p className="text-[11px] text-slate-400 font-bold italic mt-4 text-center">
										Tọa độ chính xác: {listing.lat.toFixed(4)}, {listing.lng.toFixed(4)}
									</p>
								</div>
							)}
						</motion.div>
					...
					<div className="space-y-6">
						...
						>
							...
							<div className="flex items-center gap-4 mb-8">
								<div className="w-16 h-16 bg-linear-to-br from-teal-500 to-teal-700 rounded-3xl flex items-center justify-center text-white font-black text-2xl shadow-lg shadow-teal-900/20">
							...
							<div className="space-y-3">
								...
								<a
									href={`https://zalo.me/${listing.contact_phone}`}
									target="_blank"
