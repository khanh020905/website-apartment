      ...
      <div className="flex-1 overflow-auto p-6 space-y-4">
        ...
        <div className="flex items-center justify-between gap-4">
           <div className="relative w-[320px]">
              ...
              <input type="text" placeholder="Tìm kiếm vai trò" className="w-full pl-10 pr-4 h-11 bg-white border border-slate-200 rounded-lg text-[13px] font-bold text-slate-700 outline-none focus:border-brand-primary shadow-sm" />
           </div>
           ...
           <div className="flex items-center gap-3">
              ...
              <button className="h-11 px-6 bg-brand-primary text-white rounded-lg text-[13px] font-black hover:bg-brand-dark transition-all shadow-sm flex items-center gap-2">
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm flex flex-col min-h-[600px] overflow-hidden">
                    ...
                    <React.Fragment key={mod.id}>
                       ...
                       <tr className="bg-slate-50/80 group">
                          <td className="px-5 py-2.5 border-r border-slate-100 flex items-center gap-2 cursor-pointer" onClick={() => toggleModule(mod.id)}>
                             <div className={`w-5 h-5 flex items-center justify-center rounded text-white transition-all ${expandedModules.includes(mod.id) ? 'bg-brand-primary' : 'bg-slate-300'}`}>
                          ...
                          {expandedModules.includes(mod.id) && mod.perms.map((perm) => (
                             ...
                             >
                                <td className="px-5 py-3 border-r border-slate-50 bg-white pl-12 relative group-hover:bg-slate-50 overflow-hidden">
                                   ...
                                   <div className="absolute left-8 top-1/2 -translate-y-1/2 w-1.5 h-1.5 bg-slate-200 rounded-full group-hover:bg-brand-primary transition-colors"></div>
                                </td>
                                ...
                                {ROLES.map(role => {
                                   ...
                                   return (
                                      <td key={`${perm.key}-${role.id}`} className="px-5 py-3 border-r border-slate-50 text-center bg-white cursor-pointer hover:bg-slate-50 transition-colors group-hover:bg-brand-bg/5 last:border-r-0">
                                         {renderCheck(hasPerm)}
                                      </td>
                                   )
                                })}
          ...
          <div className="px-6 py-4 bg-[#fefefe] border-t border-slate-100 flex items-center justify-end">
             <div className="flex items-center gap-1.5">
                ...
                <button className="w-10 h-10 flex items-center justify-center rounded bg-brand-primary text-white text-[14px] font-black shadow-md">1</button>
                <button className="w-10 h-10 flex items-center justify-center rounded border border-slate-200 text-slate-300">⟩</button>
             </div>
          </div>
        </div>
