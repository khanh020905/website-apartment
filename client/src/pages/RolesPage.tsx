...
const RolesPage = () => {
	...
	const MOCK_ROLES = [
		...
		},
		{
			...
			usersCount: 5,
			description: "Quản lý vận hành, khách hàng, hoá đơn của toà nhà được gán",
			type: "custom",
		},
		{
			id: "3",
			...
			usersCount: 3,
			description: "Chỉ truy cập mục sự cố và bảo trì",
			type: "custom",
		},
		{
			id: "4",
			name: "Khách hàng",
			usersCount: 120,
			description: "Truy cập ứng dụng dành cho cư dân",
			type: "system",
		},
	];
			...
			<div className="flex-1 overflow-auto bg-[#f8f9fa] p-5 lg:p-6 flex flex-col gap-6">
				...
				<div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm h-full flex flex-col min-h-100">
									...
									>
										<td className="px-5 py-4 whitespace-nowrap">
											<div className="flex items-center gap-3">
												<div
													className={`w-9 h-9 ${role.type === "system" ? "bg-indigo-50 text-indigo-600 border border-indigo-100" : "bg-slate-50 text-slate-500 border border-slate-200"} rounded-lg flex items-center justify-center shadow-sm`}
												>
										...
										<td className="px-5 py-4">
											<span className="text-[13px] text-slate-500 font-medium line-clamp-2 max-w-87.5">
												{role.description}
											</span>
										</td>
										<td className="px-5 py-4 whitespace-nowrap">
										...
										<td className="px-5 py-4 whitespace-nowrap">
											<span
												className={`inline-flex px-2 py-0.5 rounded-md text-[11px] font-bold uppercase tracking-wider border ${role.type === "system" ? "bg-indigo-50 text-indigo-700 border-indigo-200" : "bg-slate-50 text-slate-500 border-slate-200"}`}
											>
					...
					<div className="mt-auto px-5 py-3 border-t border-slate-200 bg-white flex items-center justify-end gap-4 text-[13px] text-slate-600">
						<span>
							1-{MOCK_ROLES.length}/{MOCK_ROLES.length} kết quả
						</span>
							...
							<div className="flex-1 overflow-y-auto p-5 space-y-6">
								<div>
									...
									<div className="flex items-center gap-2">
										<input
											type="date"
											className="flex-1 px-3 py-2 border border-slate-200 rounded-lg text-[13px] text-slate-600 outline-none focus:border-amber-400 transition-colors"
										/>
