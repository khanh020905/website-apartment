...
export default function OwnerReportPage() {
	...
	const fetchReports = async () => {
		setLoading(true);
		try {
			const { data } = await api.get<any[]>("/api/reports/owner");
			if (data) {
				setReports(data);
			}
		} catch (error) {
								...
								{reports.length === 0 ? (
									...
									reports.map((row, idx) => (
										<tr key={idx} className="hover:bg-slate-50 transition-colors">
											<td className="px-4 py-3 text-[13px] font-semibold text-slate-700">{row.building}</td>
											<td className="px-4 py-3 text-[13px] font-bold text-slate-900">{row.room}</td>
											<td className="px-4 py-3 text-[13px] text-slate-600">{row.room_amount.toLocaleString()} đ</td>
											<td className="px-4 py-3 text-[13px] text-slate-600">{row.old_debt.toLocaleString()} đ</td>
											<td className="px-4 py-3 text-[13px] text-slate-600">{row.deposit.toLocaleString()} đ</td>
											<td className="px-4 py-3 text-[13px] text-slate-600">{row.extras.toLocaleString()} đ</td>
											<td className="px-4 py-3 text-[13px] text-slate-600">{row.discount.toLocaleString()} đ</td>
											<td className="px-4 py-3 text-[13px] font-bold text-emerald-600">{row.owner_payout.toLocaleString()} đ</td>
											<td className="px-4 py-3 text-[13px] text-slate-600">{row.platform_collected.toLocaleString()} đ</td>
											<td className="px-4 py-3 text-[13px] font-bold text-rose-500">{row.platform_pending.toLocaleString()} đ</td>
										</tr>
									))
								)}
							</tbody>
