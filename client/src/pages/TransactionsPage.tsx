...
export default function TransactionsPage() {
  ...
  // Stats calculation
  const income = MOCK_TRANSACTIONS.filter((t) => t.flow === "income").reduce((s, t) => s + t.amount, 0);
  const expense = MOCK_TRANSACTIONS.filter((t) => t.flow === "expense").reduce((s, t) => s + t.amount, 0);
  const profit = income - expense;

  const statCards = [
    { label: "Lợi nhuận", value: profit, icon: Wallet, iconBg: "bg-violet-100", iconColor: "text-violet-600" },
    { label: "Tổng thu", value: income, icon: TrendingUp, iconBg: "bg-emerald-100", iconColor: "text-emerald-600" },
    { label: "Tổng chi", value: expense, icon: TrendingDown, iconBg: "bg-rose-100", iconColor: "text-rose-600" },
    { label: "Số giao dịch", value: MOCK_TRANSACTIONS.length, icon: ArrowLeftRight, iconBg: "bg-amber-100", iconColor: "text-amber-600", isCount: true },
  ];
      ...
      <div className="flex-1 overflow-auto bg-[#f8f9fa] p-5 lg:p-6 flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 shrink-0">
          {statCards.map((s, i) => (
            ...
            >
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${s.iconBg} transition-transform group-hover:scale-110`}>
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm h-full flex flex-col min-h-[400px]">
            ...
            <table className="w-full h-full text-left">
              <thead className="bg-[#f8f9fa] border-b border-slate-200 sticky top-0 z-10">
                <tr>
                  ...
                  {["Mã GD", "Ngày", "Toà nhà", "Phòng", "Khách hàng", "Loại GD", "Hình thức", "Số tiền", "Trạng thái", "Ghi chú"].map((h, i) => (
              ...
              <tbody className="divide-y divide-slate-100">
                {MOCK_TRANSACTIONS.length === 0 && (
                  <tr>
                    <td colSpan={12} className="px-6 py-28 text-center bg-white cursor-default">
                ...
                {MOCK_TRANSACTIONS.map((t, idx) => (
                  <tr key={t.id} className="hover:bg-amber-50/20 transition-colors group cursor-pointer">
                    ...
                    <td className="px-5 py-3.5"><span className="font-mono text-[13px] font-bold text-blue-600">{t.code}</span></td>
                    <td className="px-5 py-3.5 text-[13px] text-slate-600 font-medium">{formatDate(t.date)}</td>
                    <td className="px-5 py-3.5 text-[13px] text-slate-600 font-medium">{t.location}</td>
                    <td className="px-5 py-3.5 text-[13px] font-bold text-slate-700">{t.room || "—"}</td>
                    <td className="px-5 py-3.5 text-[13px] text-slate-600 font-medium">{t.customer || "—"}</td>
                    <td className="px-5 py-3.5 text-[13px] text-slate-600 font-medium">{t.type}</td>
                    <td className="px-5 py-3.5 text-[13px] text-slate-600 font-medium">{t.pay_type}</td>
                    <td className="px-5 py-3.5">
                      <span className={`text-[13px] font-bold ${t.flow === "income" ? "text-emerald-600" : "text-rose-600"}`}>
                        {t.flow === "income" ? "+" : "-"}{formatCurrency(t.amount)}
                      </span>
                    </td>
                    <td className="px-5 py-3.5">
                      <span className={`inline-flex px-2 py-0.5 rounded-md text-[11px] font-bold uppercase tracking-wide border ${TX_STATUS[t.status].color.replace('bg-', 'bg-opacity-20 border-').replace('text-', 'text-')}`}>
                        {TX_STATUS[t.status].label}
                      </span>
                    </td>
                    <td className="px-5 py-3.5 text-[13px] text-slate-500 max-w-[120px] truncate">{t.note || "—"}</td>
          ...
          <div className="mt-auto px-5 py-3 border-t border-slate-200 bg-white flex flex-col sm:flex-row items-center justify-between gap-4 text-[13px] text-slate-600 shrink-0">
             <span>{MOCK_TRANSACTIONS.length === 0 ? "0-0/0" : `1-${MOCK_TRANSACTIONS.length}/${MOCK_TRANSACTIONS.length}`} kết quả</span>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-6">
                 ...
                 <div>
                   ...
                   <div className="flex items-center gap-6">
                     <label className="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="flow" className="w-4 h-4 text-amber-500 focus:ring-amber-500 border-slate-300" />
                 ...
                 <div>
                   ...
                   <div className="grid grid-cols-2 gap-3">
                     {["Thanh toán đặt phòng", "Nhập quỹ", "Rút tiền", "Thuê ngoài", "Khác"].map(st => (
                 ...
                 <div>
                   <h3 className="text-[13px] font-bold text-slate-900 mb-3">Hình thức thanh toán</h3>
