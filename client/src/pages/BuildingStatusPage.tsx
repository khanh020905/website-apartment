  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center pb-20">
      <div className="w-full max-w-md bg-white border-b border-slate-100 p-8 shadow-sm relative overflow-hidden">
        {/* HomeSpot Branding */}
        <Link to="/" className="absolute top-4 left-6 flex items-center gap-2 group">
          ...
          <span className="text-[10px] font-black text-slate-900 uppercase tracking-widest opacity-40 group-hover:opacity-100 transition-opacity">HomeSpot</span>
        </Link>
             ...
             >
               ...
               <div className="relative z-10">
                 <div className="flex items-center gap-4 mb-8">
                   <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg ${PUBLIC_STATUS_CONFIG[selectedRoom.status].bg} ${PUBLIC_STATUS_CONFIG[selectedRoom.status].border} border-2`}>
                      <span className={`text-xl font-black ${PUBLIC_STATUS_CONFIG[selectedRoom.status].color}`}>{selectedRoom.room_number}</span>
                   </div>
                   <div>
                      <h3 className="text-xl font-black text-slate-900 leading-none mb-1">Chi tiết phòng</h3>
                      <span className={`text-[10px] font-black uppercase tracking-widest ${PUBLIC_STATUS_CONFIG[selectedRoom.status].color}`}>
                        {selectedRoom.status === 'available' ? 'Đang trống' : selectedRoom.status === 'occupied' ? 'Đã có người' : 'Bảo trì'}
                      </span>
                   </div>
                 </div>
                 ...
                 <div className="grid grid-cols-2 gap-4 mb-8">
                    <div className="p-4 bg-slate-50 rounded-3xl border border-slate-100">
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 opacity-50">Giá thuê</p>
                       <p className="text-sm font-black text-slate-900">{((selectedRoom.price || 0) / 1000000).toFixed(1)} <span className="text-xs">Trđ/th</span></p>
                    </div>
                    <div className="p-4 bg-slate-50 rounded-3xl border border-slate-100">
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 opacity-50">Diện tích</p>
                       <p className="text-sm font-black text-slate-900">{selectedRoom.area} m²</p>
                    </div>
                    <div className="p-4 bg-slate-50 rounded-3xl border border-slate-100">
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 opacity-50">Sức chứa</p>
                       <p className="text-sm font-black text-slate-900">{selectedRoom.max_occupants || '--'} người</p>
                    </div>
                    <div className="p-4 bg-slate-50 rounded-3xl border border-slate-100">
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 opacity-50">Hiện tại</p>
                       <p className="text-sm font-black text-slate-900">{selectedRoom.current_occupants || 0} người</p>
                    </div>
                 </div>
                 ...
                 {selectedRoom.status === 'available' ? (
                   ...
                   <div className="py-4 text-center border-2 border-dashed border-slate-100 rounded-[24px]">
                     <p className="text-xs font-bold text-slate-300 italic uppercase tracking-widest">Phòng này hiện không sẵn sàng</p>
                   </div>
                 )}
               </div>
             </motion.div>
