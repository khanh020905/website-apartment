...
export default function CustomerPage() {
  ...
  useEffect(() => {
    setPage(1);
  }, [debouncedSearch, filterStatus, filterGender, filterResidency, filterRoom, selectedBuildingId]);
  ...
  const fetchCustomers = useCallback(async () => {
    ...
    setLoading(false);
  }, [page, limit, debouncedSearch, filterStatus, filterGender, filterResidency, selectedBuildingId]);
      ...
      <div className="flex-1 overflow-auto bg-[#f8f9fa] p-6">
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm h-full flex flex-col">
            ...
            <table className="w-full">
              <thead className="bg-[#f8f9fa] border-b border-slate-200">
                <tr>
                  ...
                  {["Khách hàng", "Phòng", "Trạng thái khách hàng", "Toà nhà", "Trạng thái tạm trú", "Giới tính", "Số CCCD/Hộ chiếu"].map((h, i) => (
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan={9} className="px-6 py-28 text-center bg-white">
                  ...
                  <tr>
                    <td colSpan={9} className="px-6 py-28 text-center bg-white cursor-default">
                    ...
                    >
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-bold text-slate-900 truncate max-w-[150px]">{c.tenant_name}</p>
                            <p className="text-[11px] text-slate-500 font-medium">{c.tenant_phone}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-3">
                      ...
                      <td className="px-5 py-3">
                        <span className={`inline-flex px-2 py-1 rounded-md text-[11px] font-bold whitespace-nowrap ${
                      ...
                      <td className="px-5 py-3">
                         <div className="flex items-center gap-2 text-xs font-semibold text-slate-600">
                           ...
                           <span className="truncate max-w-[120px]">{c.room?.building?.name}</span>
                         </div>
                      </td>
                      <td className="px-5 py-3">
                      ...
                      <td className="px-5 py-3 text-sm text-slate-600 font-medium whitespace-nowrap">
                         {c.tenant_gender === "male" ? "Nam" : c.tenant_gender === "female" ? "Nữ" : c.tenant_gender === "other" ? "Khác" : "-"}
                      </td>
                      <td className="px-5 py-3 text-sm text-slate-600 font-medium whitespace-nowrap">
                         {c.tenant_id_number || "-"}
                      </td>
          ...
          <div className="mt-auto px-5 py-3 border-t border-slate-200 bg-white flex items-center justify-end gap-4 text-sm text-slate-600">
             <span>
               {total === 0 ? "0-0/0" : `${(page - 1) * limit + 1}-${Math.min(page * limit, total)}/${total}`}
             </span>
