...
import { 
  ...
  Lock, Key, Save, AlertCircle, CheckCircle2 
} from 'lucide-react';
      ...
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-500 font-medium mt-1">Quản lý thông tin cá nhân và cài đặt bảo mật của bạn.</p>
          </div>
          <div className={`px-4 py-2 rounded-full text-xs font-black uppercase tracking-widest flex items-center gap-2 border-2 ${
            user?.profile?.is_verified ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : 'bg-slate-100 border-slate-200 text-slate-400'
          }`}>
            <Shield size={14} className={user?.profile?.is_verified ? 'fill-emerald-600/20' : ''} />
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            >
              <div className="relative inline-block mb-6">
                <div className="w-32 h-32 rounded-[32px] bg-slate-100 border-4 border-white shadow-xl overflow-hidden">
                  <img 
                    src={user?.profile?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.profile?.full_name || 'User')}&background=random`} 
                    alt="Avatar" 
              ...
              <h2 className="text-xl font-black text-slate-900 leading-tight truncate" title={user?.profile?.full_name || ''}>{user?.profile?.full_name || 'Chưa cập nhật tên'}</h2>
              <p className="text-slate-400 font-bold text-xs uppercase tracking-widest mt-1">
                {user?.profile?.role === 'landlord' ? 'Chủ nhà' : user?.profile?.role === 'broker' ? 'Môi giới' : 'Khách hàng'}
              </p>
              ...
              <div className="mt-8 pt-8 border-t border-slate-50 flex flex-col gap-4 text-left">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest leading-none mb-1">Điện thoại</p>
            ...
            >
              ...
              <form onSubmit={handleUpdateProfile} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Ngày sinh</label>
                    <div className="relative">
                      <input 
                        type="date" 
                        value={dob}
