...
export default function UsersPage() {
  ...
  const ROLES = [
    { id: 1, name: "Quản lý hệ thống", usersCount: 2, desc: "Quản lý tất cả hệ thống" },
    { id: 2, name: "Chủ toà nhà", usersCount: 3, desc: "Chủ đầu tư hoặc giám đốc các toà nhà" },
    { id: 3, name: "Ban quản lý", usersCount: 5, desc: "Quản lý vận hành khu vực" },
    { id: 4, name: "Lễ tân / CSKH", usersCount: 6, desc: "Hỗ trợ khách hàng" },
    { id: 5, name: "Kế toán", usersCount: 2, desc: "Quản trị dòng tiền" },
    { id: 6, name: "Cư dân / Người thuê", usersCount: 156, desc: "Tài khoản khách thuê phòng" },
  ];
          ...
          <div className="flex gap-10 overflow-x-auto scrollbar-hide">
             {TABS.map(tab => (
               <button 
                 ...
                 className={`text-[15px] font-black pb-3 border-b-2 whitespace-nowrap transition-colors outline-none px-1 cursor-pointer ${
                   activeTab === tab.id ? 'text-[#14B8A6] border-[#14B8A6]' : 'text-slate-400 border-transparent hover:text-slate-600'
                 }`}
               >
                 {tab.label}
               </button>
          ...
          <div className="space-y-4 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex flex-wrap items-center gap-3">
               <div className="relative flex-1 min-w-[200px]">
                  ...
                  <input type="text" placeholder="Tên" className="w-full pl-10 pr-4 h-10 bg-white border border-slate-200 rounded-lg text-[13px] font-bold text-slate-700 outline-none focus:border-[#14B8A6] shadow-sm" />
               </div>
               <input type="text" placeholder="Email" className="w-[180px] px-3 h-10 bg-white border border-slate-200 rounded-lg text-[13px] font-bold text-slate-700 outline-none focus:border-[#14B8A6] shadow-sm" />
               
               <div className="w-[180px] relative group">
                  <select className="w-full pl-3 pr-8 h-10 bg-white border border-slate-200 rounded-lg text-[13px] font-bold text-slate-700 outline-none appearance-none focus:border-[#14B8A6] shadow-sm cursor-pointer">
               ...
               <div className="overflow-x-auto">
                  <table className="w-full text-left">
                     ...
                     <tbody className="divide-y divide-slate-100">
                        {USERS.map((user) => (
                           <tr key={user.id} className="hover:bg-[#14B8A6]/5 transition-colors group">
                              ...
                              <td className="px-5 py-3">
                                 <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button className="p-1.5 hover:bg-slate-100 rounded text-blue-500 cursor-pointer"><Mail className="w-4 h-4" /></button>
                                    <button className="p-1.5 hover:bg-slate-100 rounded text-slate-500 cursor-pointer"><Edit2 className="w-4 h-4" /></button>
                                    <button className="p-1.5 hover:bg-slate-100 rounded text-rose-500 cursor-pointer"><Trash2 className="w-4 h-4" /></button>
                                 </div>
                              </td>
                           </tr>
                        ))}
          ...
          <div className="space-y-4 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between">
               <div className="relative w-[300px]">
                  <Search className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" />
                  <input type="text" placeholder="Tìm kiếm vai trò" className="w-full pl-10 pr-4 h-10 bg-white border border-slate-200 rounded-lg text-[13px] font-bold text-slate-700 outline-none focus:border-[#14B8A6] shadow-sm" />
               </div>
               <button className="h-10 px-5 bg-brand-primary text-white rounded-lg text-[13px] font-black hover:bg-brand-dark transition-colors shadow-sm cursor-pointer flex items-center gap-1.5">
                  <Plus className="w-4.5 h-4.5" strokeWidth={3} /> Thêm vai trò
               </button>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
               <div className="overflow-x-auto">
                  <table className="w-full text-left">
                     <thead>
                        <tr className="bg-[#EDEDED]">
                           {["Tên vai trò", "Mô tả", "Số tài khoản", "Hành động"].map((h, i) => (
                              <th key={i} className={`px-5 py-3 text-[11px] font-black text-slate-700 uppercase tracking-wider border-r border-white last:border-0 ${i === 2 ? 'text-center' : ''}`}>{h}</th>
                           ))}
                        </tr>
                     </thead>
                     <tbody className="divide-y divide-slate-100">
                        {ROLES.map((role) => (
                           <tr key={role.id} className="hover:bg-[#14B8A6]/5 transition-colors group">
                              <td className="px-5 py-4 text-[13px] font-bold text-slate-900">{role.name}</td>
                              <td className="px-5 py-4 text-[13px] text-slate-500">{role.desc}</td>
                              <td className="px-5 py-4 text-center">
                                 <span className="inline-flex items-center justify-center px-2.5 py-1 rounded bg-[#14B8A6]/10 text-[#14B8A6] text-[12px] font-black">
                                    {role.usersCount}
                                 </span>
                              </td>
                              <td className="px-5 py-4">
                                 <div className="flex items-center gap-3">
                                    <button className="text-[13px] font-bold text-blue-500 hover:text-blue-700 transition-colors cursor-pointer">Phân quyền</button>
                                    <button className="text-[13px] font-bold text-slate-500 hover:text-slate-700 transition-colors cursor-pointer">Sửa</button>
                                    <button className="text-[13px] font-bold text-rose-500 hover:text-rose-700 transition-colors cursor-pointer">Xoá</button>
                                 </div>
                              </td>
                           </tr>
                        ))}
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-6 pb-24">
                     <div className="flex flex-col items-center gap-3">
                        <div className="w-24 h-24 rounded-full bg-slate-50 border-2 border-dashed border-slate-200 flex items-center justify-center relative cursor-pointer hover:bg-slate-100 transition-colors">
                           ...
                           <div className="absolute bottom-0 right-0 bg-white p-1.5 rounded-full shadow-lg border border-slate-100 text-[#14B8A6]"><Plus className="w-3.5 h-3.5 font-black" /></div>
                        </div>
                     ...
                     <div className="grid grid-cols-1 gap-5">
                        <div className="space-y-1.5">
                           ...
                           <select className="w-full px-4 h-11 border border-slate-200 rounded-lg text-sm font-bold text-slate-800 focus:border-[#14B8A6] outline-none appearance-none bg-white">
                              <option>Chọn vai trò</option>
                           </select>
                        </div>
                        <div className="space-y-1.5">
                           <label className="text-[13px] font-black text-slate-700 uppercase">Tên đầy đủ</label>
                           <input type="text" placeholder="Nhập tên đầy đủ" className="w-full px-4 h-11 border border-slate-200 rounded-lg text-sm font-bold text-slate-800 focus:border-[#14B8A6] outline-none" />
                        </div>
                        <div className="space-y-1.5">
                           <label className="text-[13px] font-black text-slate-700 uppercase">Email</label>
                           <input type="email" placeholder="Nhập email" className="w-full px-4 h-11 border border-slate-200 rounded-lg text-sm font-bold text-slate-800 focus:border-[#14B8A6] outline-none" />
                        </div>
                        <div className="space-y-1.5">
                           <label className="text-[13px] font-black text-slate-700 uppercase">Số điện thoại</label>
                           <input type="text" placeholder="Nhập số điện thoại" className="w-full px-4 h-11 border border-slate-200 rounded-lg text-sm font-bold text-slate-800 focus:border-[#14B8A6] outline-none" />
                        </div>
                        <div className="space-y-1.5">
