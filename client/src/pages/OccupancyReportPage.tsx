...
const OccupancyReportPage = () => {
  ...
  const STATS = [
    { label: "Tỷ lệ lấp đầy", value: "0%", icon: Percent, color: "text-amber-500", bg: "bg-amber-50" },
    { label: "Phòng đang sử dụng / Tổng phòng", value: "0/18", icon: Home, color: "text-blue-500", bg: "bg-blue-50" },
    { label: "Phòng trống / Tổng phòng", value: "18/18", icon: Home, color: "text-slate-500", bg: "bg-slate-50" },
    { label: "Nhận phòng tháng này", value: "0", icon: LogIn, color: "text-emerald-500", bg: "bg-emerald-50" },
  ];
    ...
    <div className="flex-1 flex flex-col h-full bg-[#f8f9fa] relative overflow-hidden">
      ...
      <div className="bg-white border-b border-slate-200">
        ...
        <div className="px-6 flex items-center justify-between border-t border-slate-100 py-3">
          <div className="flex gap-6">
            <button className="text-[14px] font-bold text-amber-600 border-b-2 border-amber-500 pb-3 -mb-3">Vận hành</button>
          </div>
      ...
      <div className="flex-1 overflow-auto bg-[#f8f9fa] p-5 lg:p-6 flex flex-col gap-6">
        ...
        <div className="text-[13px] text-slate-500 font-medium">
          Cập nhật lần cuối lúc: 07/04/2026 02:58 <RefreshCw className="w-3 h-3 inline-block ml-1 cursor-pointer hover:text-slate-900" />
        </div>
          ...
          {STATS.map((stat, i) => (
            <div key={i} className="bg-white px-5 py-4 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-center gap-1.5 min-h-[90px]">
              <div className="flex items-start gap-4">
                 <div className={`w-10 h-10 ${stat.bg} rounded-full flex items-center justify-center shrink-0`}>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col h-[400px]">
          <div className="p-5 border-b border-slate-100 flex items-center justify-between">
            <h3 className="text-[14px] font-bold text-slate-800">
               Tỷ lệ lấp đầy theo thời gian <span className="text-blue-500 font-medium">(trung bình 0%)</span>
            </h3>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col h-[400px]">
          <div className="p-5 border-b border-slate-100 flex items-center justify-between">
            <h3 className="text-[14px] font-bold text-slate-800">
               Tỷ lệ lấp đầy theo tòa nhà <span className="text-blue-500 font-medium">(trung bình 0%)</span>
            </h3>
            <div className="flex items-center gap-2 opacity-50">
               <button className="p-1 hover:bg-slate-100 rounded text-slate-500"><Maximize2 className="w-4 h-4" /></button>
               <button className="p-1 hover:bg-slate-100 rounded text-slate-500"><Download className="w-4 h-4" /></button>
