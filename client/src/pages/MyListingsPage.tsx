            ...
            {filtered.map((listing, i) => (
              <motion.div key={listing.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <div className="w-48 h-36 flex-shrink-0">
                  <img
                    src={listing.images?.[0]?.url || listing.images?.[0] as unknown as string || 'https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=300&h=200&fit=crop'}
                    alt={listing.title}
                ...
                <div className="flex-1 p-4 flex flex-col">
                  <div className="flex items-start justify-between">
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${STATUS_STYLES[listing.status].bg} ${STATUS_STYLES[listing.status].text}`}>
