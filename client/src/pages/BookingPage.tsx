	...
	return (
		<div className="flex flex-col h-full bg-white text-slate-800">
			...
			<div className="flex items-center px-6 py-3 gap-3 bg-slate-50/30">
				{/* Date */}
				<label className="relative flex items-center gap-2 px-3 py-2 border border-slate-200 rounded-xl text-sm text-slate-600 bg-white min-w-37.5 cursor-pointer hover:border-amber-400 transition-all font-medium">
				...
				{/* Floor Select */}
				<div className="relative border border-slate-200 rounded-xl bg-white min-w-37.5 hover:border-amber-400 transition-all">
					<select
						value={selectedFloor}
						onChange={(e) =>
			...
			<div className="flex-1 p-6 overflow-y-auto">
				<div className="border border-slate-200 bg-white rounded-2xl flex flex-col shadow-sm overflow-hidden min-h-125">
					{/* Table Header & Legend */}
						...
						<div className="flex items-center gap-6 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
							{(Object.entries(STATUS) as [RoomStatus, (typeof STATUS)[RoomStatus]][]).map(
								([, cfg]) => (
									...
									>
										<span
											className={`w-3 h-3 rounded-full border-2 bg-white ${cfg.borderColorClass}`}
										/>
										<span>{cfg.label}</span>
									</div>
								),
					...
					:	<div className="divide-y divide-slate-100">
							{sortedFloors.map((floor) => {
								...
								return (
									<div
										key={floor}
										className="flex min-h-35 group transition-colors hover:bg-slate-50/30"
									>
