	...
	const MOCK_TOKENS = [
		{
			...
			expires_at: "2324-04-06",
			status: "active",
		},
		{
			id: "2",
			name: "Internal ERP Connector",
			secret: "sk_live_72N...",
			...
			<div className="flex-1 overflow-auto bg-[#f8f9fa] p-5 lg:p-6 flex flex-col gap-6">
				...
				<div className="flex items-center gap-3 bg-amber-50 p-4 rounded-xl border border-amber-100 shrink-0">
					...
					<p className="text-[13px] text-amber-800 leading-relaxed font-medium">
						Lưu ý quan trọng: Hệ thống không lưu trữ API Secret Code của bạn sau khi khởi tạo. Không
						bao giờ chia sẻ API Secret của bạn với bất kỳ ai để đảm bảo an toàn.
					</p>
				</div>
				...
				<div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm h-full flex flex-col min-h-100">
					<div className="overflow-x-auto flex-1">
						<table className="w-full text-left h-full">
							<thead className="bg-[#f8f9fa] border-b border-slate-200 sticky top-0 z-10">
								<tr>
									{["Ứng dụng", "API Key / Token", "Ngày tạo", "Hết hạn", "Trạng thái"].map(
										(h, i) => (
											<th
												key={i}
									...
									>
										<td className="px-5 py-4 whitespace-nowrap">
											<div className="flex items-center gap-3">
												...
												<span className="text-[13px] font-bold text-slate-900 group-hover:text-amber-600 transition-colors cursor-pointer">
													{token.name}
												</span>
											</div>
										</td>
										<td className="px-5 py-4 whitespace-nowrap">
											<div className="flex items-center gap-2 font-mono text-[13px] bg-slate-50 px-3 py-1.5 rounded border border-slate-200 text-slate-600 w-fit">
												...
												<button
													onClick={() => toggleShow(token.id)}
													className="p-1 hover:bg-white border border-transparent hover:border-slate-200 shadow-sm rounded shadow-black/5 text-slate-400 hover:text-blue-600 transition-all cursor-pointer"
												>
													{showSecret[token.id] ?
					...
					<div className="mt-auto px-5 py-3 border-t border-slate-200 bg-white flex flex-col sm:flex-row items-center justify-between gap-4 text-[13px] text-slate-600 shrink-0">
						<div className="flex items-center gap-2 text-[12px] font-bold text-slate-500 whitespace-nowrap">
							...
							Lịch sử truy cập API gần nhất:{" "}
							<span className="text-slate-900 uppercase">Vừa xong (12:05 PM)</span>
						</div>
