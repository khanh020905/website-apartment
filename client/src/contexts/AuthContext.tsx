...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
	...
	const signIn = async (email: string, password: string) => {
		const { data, error } = await api.post<{ user: User; session: Session }>("/api/auth/login", {
			email,
			password,
		});
	...
	const signUp = async (
		...
		const { data, error } = await api.post<{ user: User; session: Session }>("/api/auth/register", {
			email,
			password,
			fullName,
			role,
	...
	const updateProfile = async (profileData: Partial<Profile>) => {
		const { error } = await api.put<{ profile: Profile }>("/api/auth/profile", profileData);
	...
	const upgradeAccount = async (
		...
		const { error } = await api.put<{ profile: Profile }>("/api/auth/upgrade", {
			role: newRole,
